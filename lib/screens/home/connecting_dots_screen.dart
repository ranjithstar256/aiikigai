...
class _ConnectingDotsScreenState extends ConsumerState<ConnectingDotsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _generateInsights() async {
    ...
    try {
      ...
      final answersState = ref.read(answersProvider);
      final questionsState = ref.read(questionsProvider);
      final formattedAnswers = answersState.formatAnswersForAI(questionsState.sections);
      ...
      ref.read(analyticsServiceProvider).logInsightGenerated(
        ref.read(authProvider).user?.isPremium ?? true,
      );
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
      ...
      // Try to save directly using FlutterFileDialog first
      try {
        final params = SaveFileDialogParams(
          data: bytes,
          fileName: 'ikigai_insights_${DateTime.now().millisecondsSinceEpoch}.pdf',
        ...
        print("FlutterFileDialog failed: $dialogError");
        // Continue to alternative method if this fails
      }

      // Alternative: Save to temporary directory and then share
      try {
        ...
        print("Share method failed: $shareError");
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to share PDF: $shareError'))
          );
        }
      }
    } catch (e) {

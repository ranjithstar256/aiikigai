...
import '../../widgets/ikigai/section_card.dart';
import '../../widgets/premium/premium_banner.dart';
import '../../data/services/firebase_service.dart'; // Import FirebaseService
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Future<void> _testFirebaseConnection() async {
    ...
    // Define dummy sections
    final dummySections = [
      Section(
        id: 'test_love',
        title: 'Test Love Section',
        description: 'A test section for love.',
        iconName: 'favorite',
        questions: [
          Question(id: 'q1', text: 'What do you love doing?'),
          Question(id: 'q2', text: 'What makes you happy?'),
        ],
        isPremium: false,
        order: 1,
      ),
      Section(
        id: 'test_strengths',
        title: 'Test Strengths Section',
        description: 'A test section for strengths.',
        iconName: 'star',
        questions: [
          Question(id: 'q1', text: 'What are you good at?'),
          Question(id: 'q2', text: 'What’s your top skill?'),
        ],
        isPremium: false,
        order: 2,
      ),
    ...
    try {
      final firebaseService = ref.read(firebaseServiceProvider);
      final dummySections = firebaseService._createDefaultSections(); // Use full defaults
      // Save each section to Firestore
      for (var section in dummySections) {
        await firebaseService._sectionsCollection.doc(section.id).set({
          'id': section.id,
          'title': section.title,
          'description': section.description,
          'iconName': section.iconName,
          'questions': section.questions.map((q) => {'id': q.id, 'text': q.text}).toList(),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Dummy sections saved to Firebase!')),
      );
    } catch (e) {
      print('Error saving dummy sections: $e');
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          children: [
            ...
            Expanded(
              child: sections.isEmpty
                ...
                itemBuilder: (context, index) {
                  final section = sections[index];
                  final isComplete = ref.watch(sectionCompleteProvider(section));
                  return SectionCard(
                    section: section,
                    isCompleted: isComplete,
                    onTap: () => _navigateToSection(section),
                ...
                children: [
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(vertical: 15, horizontal: 40),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)),
                      backgroundColor: Colors.green, // Distinct color for testing
                    ),
                    child: Text(
                      "Test Firebase",
                      style: theme.textTheme.labelLarge?.copyWith(
